...
> **v9 (Aug 2026, premium persuasive integration):** the box-soup bridge/close was replaced with the **Fladlien paradigm-shift** narrative (`PATHWAY_PITCH`, one per pathway), rendered as a premium paced layout (`.pv-*`): compact cinematic gradient hero → paradigm shift (flip the belief, remove self-blame) → the blocks installed young to 'keep you safe' (quiet italic lines) → a visceral test (mystic = the $1M-in-your-account reaction) → the reframe ('release the old you') → mechanism + real in-pathway proof → CTA. Whitespace-led, no container-soup, no emoji, per-pathway color world. The offer screen was decluttered (removed value-stack + why-yearly boxes); keeps cinematic program-cover cards, tiers, timer, guarantee, wall, exit ramp, checkout. Rendered/verified with a real browser (puppeteer) across all six pathways. Standalone references: close-persuasive.html, bridge-redesign.html.

> **v8 (Aug 2026):** (1) **Removed all $5K-product testimonials** — Business Mastery / Entrepreneurship Mastery / AI Mastery are NOT in membership or any pathway, so they can't be proof for a $199 pathway. Replaced EE wall + proof with real in-pathway students sourced from stories.mindvalley.com (Vivid Vision — Lucille Orr; Healing Burnout — Dao Lam; The Power of Boldness — Monica Ducu & Kanika Jain; Money EQ — Audrey, Chris). (2) **Milestones now show the HOW** — each month-3/6/12 promise carries a `horizon-how` line naming the exact programs that deliver it, via a shared `buildRoadmap()` used by both bridge and plan (month 3 = the 3 curriculum programs; month 6 = named backup programs; month 12 = the full N-program pathway). No more fluffy promises. (3) **Ledger reframed to outcomes** — headers 'Leave behind / Start experiencing', items are concrete expectable results, not process/identity language.
